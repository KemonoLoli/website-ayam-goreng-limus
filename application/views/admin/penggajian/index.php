<?php
$bulan_nama = [
    '',
    'Januari',
    'Februari',
    'Maret',
    'April',
    'Mei',
    'Juni',
    'Juli',
    'Agustus',
    'September',
    'Oktober',
    'November',
    'Desember'
];
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="page-title"><?= $page_title ?></h1>
    <div>
        <button class="btn btn-primary" data-coreui-toggle="modal" data-coreui-target="#generateModal">
            <i class="cil-sync me-1"></i> Generate Gaji
        </button>
    </div>
</div>

<!-- Period Filter -->
<div class="card mb-4">
    <div class="card-body py-3">
        <form method="get" class="row g-3 align-items-end">
            <div class="col-md-2">
                <label class="form-label">Bulan</label>
                <select name="bulan" class="form-select form-select-sm">
                    <?php for ($i = 1; $i <= 12; $i++): ?>
                        <option value="<?= str_pad($i, 2, '0', STR_PAD_LEFT) ?>" <?= $bulan == $i ? 'selected' : '' ?>>
                            <?= $bulan_nama[$i] ?>
                        </option>
                    <?php endfor; ?>
                </select>
            </div>
            <div class="col-md-2">
                <label class="form-label">Tahun</label>
                <select name="tahun" class="form-select form-select-sm">
                    <?php for ($y = date('Y'); $y >= 2020; $y--): ?>
                        <option value="<?= $y ?>" <?= $tahun == $y ? 'selected' : '' ?>><?= $y ?></option>
                    <?php endfor; ?>
                </select>
            </div>
            <div class="col-md-2">
                <label class="form-label">Status</label>
                <select name="status" class="form-select form-select-sm">
                    <option value="">Semua</option>
                    <option value="draft" <?= $filters['status'] === 'draft' ? 'selected' : '' ?>>Draft</option>
                    <option value="disetujui" <?= $filters['status'] === 'disetujui' ? 'selected' : '' ?>>Disetujui
                    </option>
                    <option value="dibayar" <?= $filters['status'] === 'dibayar' ? 'selected' : '' ?>>Dibayar</option>
                </select>
            </div>
            <div class="col-auto">
                <button type="submit" class="btn btn-sm btn-primary"><i class="cil-search"></i> Filter</button>
            </div>
        </form>
    </div>
</div>

<!-- Summary Cards -->
<?php if ($summary): ?>
    <div class="row mb-4">
        <div class="col-md-3">
            <div class="card bg-primary text-white">
                <div class="card-body">
                    <div class="fs-5 fw-bold"><?= $summary->total_karyawan ?></div>
                    <div class="small">Total Karyawan</div>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card bg-success text-white">
                <div class="card-body">
                    <div class="fs-5 fw-bold"><?= format_rupiah($summary->total_gaji_bersih) ?></div>
                    <div class="small">Total Gaji Bersih</div>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card bg-info text-white">
                <div class="card-body">
                    <div class="fs-5 fw-bold"><?= $summary->dibayar ?> / <?= $summary->total_karyawan ?></div>
                    <div class="small">Sudah Dibayar</div>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card bg-warning text-dark">
                <div class="card-body">
                    <div class="fs-5 fw-bold"><?= $summary->draft ?></div>
                    <div class="small">Menunggu Approval</div>
                </div>
            </div>
        </div>
    </div>
<?php endif; ?>

<!-- Payroll List -->
<div class="card">
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                <thead class="table-light">
                    <tr>
                        <th>Karyawan</th>
                        <th>Jabatan</th>
                        <th class="text-end">Gaji Pokok</th>
                        <th class="text-end">Tunjangan</th>
                        <th class="text-end">Potongan</th>
                        <th class="text-end">Gaji Bersih</th>
                        <th>Status</th>
                        <th width="100">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (empty($gaji_list)): ?>
                        <tr>
                            <td colspan="8" class="text-center py-4 text-body-secondary">
                                Belum ada data penggajian untuk periode ini. Klik "Generate Gaji" untuk membuat.
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($gaji_list as $g): ?>
                            <tr>
                                <td>
                                    <div class="fw-semibold"><?= $g->nama ?></div>
                                    <small class="text-body-secondary"><?= $g->nip ?></small>
                                </td>
                                <td><?= $g->jabatan ?></td>
                                <td class="text-end"><?= format_rupiah($g->gaji_pokok) ?></td>
                                <td class="text-end text-success">+<?= format_rupiah($g->total_tunjangan) ?></td>
                                <td class="text-end text-danger">-<?= format_rupiah($g->total_potongan) ?></td>
                                <td class="text-end fw-bold"><?= format_rupiah($g->gaji_bersih) ?></td>
                                <td>
                                    <?php
                                    $colors = ['draft' => 'warning', 'disetujui' => 'info', 'dibayar' => 'success'];
                                    ?>
                                    <span
                                        class="badge bg-<?= $colors[$g->status] ?? 'secondary' ?>"><?= ucfirst($g->status) ?></span>
                                </td>
                                <td>
                                    <a href="<?= admin_url('penggajian/detail/' . $g->id_penggajian) ?>"
                                        class="btn btn-sm btn-outline-primary">
                                        <i class="cil-arrow-right"></i>
                                    </a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    </div>
</div>

<!-- Generate Modal -->
<div class="modal fade" id="generateModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" action="<?= admin_url('penggajian/generate') ?>">
                <?= csrf_field() ?>
                <div class="modal-header">
                    <h5 class="modal-title">Generate Penggajian</h5>
                    <button type="button" class="btn-close" data-coreui-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label class="form-label">Bulan</label>
                            <select name="bulan" class="form-select">
                                <?php for ($i = 1; $i <= 12; $i++): ?>
                                    <option value="<?= str_pad($i, 2, '0', STR_PAD_LEFT) ?>" <?= $bulan == $i ? 'selected' : '' ?>>
                                        <?= $bulan_nama[$i] ?>
                                    </option>
                                <?php endfor; ?>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Tahun</label>
                            <select name="tahun" class="form-select">
                                <?php for ($y = date('Y'); $y >= 2020; $y--): ?>
                                    <option value="<?= $y ?>" <?= $tahun == $y ? 'selected' : '' ?>><?= $y ?></option>
                                <?php endfor; ?>
                            </select>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Karyawan</label>
                        <select name="id_karyawan" class="form-select">
                            <option value="">-- Semua Karyawan Aktif --</option>
                            <?php foreach ($karyawan_list as $k): ?>
                                <option value="<?= $k->id_karyawan ?>"><?= $k->nama ?> (<?= $k->jabatan ?>)</option>
                            <?php endforeach; ?>
                        </select>
                        <small class="text-body-secondary">Kosongkan untuk generate semua karyawan sekaligus</small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-coreui-dismiss="modal">Batal</button>
                    <button type="submit" class="btn btn-primary">Generate</button>
                </div>
            </form>
        </div>
    </div>
</div>